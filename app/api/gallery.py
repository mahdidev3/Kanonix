...
@router.post("/upload")
async def upload(event_id: int = Form(...), title: str = Form(""), file: UploadFile = File(...), session: AsyncSession = Depends(get_session), _=Depends(require_role(Role.admin, Role.superadmin)), kanoon=Depends(resolve_tenant)):
